...
const Insight = () => {
  ...
  const [muscleFatigueIndexData, setMuscleFatigueIndexData] = useState([]);
        ...
        const fetchMuscleFatigueIndexData = async () => {
            ...
            onAuthStateChanged(auth, async (user) => {
                if (user) {
                    ...
                    const fatigueIndexData = [];

                    for (const trainingId in trainingNames) {
                        ...
                        // Fetch the muscle fatigue index data for each training
                        onValue(mfiRef, (snapshot) => {
                            const data = snapshot.val();
                            if (data) {
                                // Push an object containing training name and its muscle fatigue index data
                                fatigueIndexData.push({ trainingName, data });
                                setMuscleFatigueIndexData([...fatigueIndexData]);
                            }
                        });
                    }
                }
    ...
    // Mapping colors to each muscle group
    const muscleColors = {
        L_quads: 'palevioletred',
        L_hams: 'mediumspringgreen',
        L_glutes: 'mediumpurple',
        R_quads: 'palevioletred',
        R_hams: 'mediumspringgreen',
        R_glutes: 'mediumpurple'
    };
...
<View>
            <Text>Muscle Fatigue Index Data</Text>
            <View>
                {muscleFatigueIndexData.map((trainingData, index) => (
                    <View key={index}>
                        <Text style={{ color: 'white' }}>{trainingData.trainingName}</Text>
                        <View style={{ flexDirection: 'row', height: 200 }}>
                            <YAxis
                                data={Object.values(trainingData.data)}
                                contentInset={{ top: 30, bottom: 30 }}
                                svg={{ fontSize: 10, fill: 'white' }}
                                numberOfTicks={Object.keys(trainingData.data).length}
                            />
                            <BarChart
                        ...
                        <XAxis
                            data={Object.values(trainingData.data)}
                            formatLabel={(value, index) => Object.keys(trainingData.data)[index]}
  ...
  gridMin={0}
  gridMax={100}
  data={muscleFatigueIndexData.map((trainingData, index) => {
    const muscleFatigueValues = Object.values(trainingData.data); // Get the values of muscle fatigue index
    return {
    data: muscleFatigueValues,
    svg: { 
        fill: muscleFatigueValues.map((value, index) => muscleColors[Object.keys(trainingData.data)[index]]) 
    } 
};
  })}
  contentInset={{ top: 30, bottom: 30 }}
